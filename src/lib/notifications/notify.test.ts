...
describe("notify", () => {
  it("no-ops when the workflow config is disabled", async () => {
    findUniqueMock.mockResolvedValue({ key: "generic-alert", enabled: false, channels: ["in_app"] });
    await notify("generic-alert", { userSub: "u1", data: { title: "T", body: "B" } });
  ...
  it("no-ops when there is no config row", async () => {
    ...
    await notify("generic-alert", { userSub: "u1", data: {} });
    expect(createMock).not.toHaveBeenCalled();
  });

  it("inserts an in-app notification when enabled with in_app channel", async () => {
    findUniqueMock.mockResolvedValue({ key: "generic-alert", enabled: true, channels: ["in_app"] });
    await notify("generic-alert", { userSub: "u1", data: { title: "T", body: "B" } });
    expect(createMock).toHaveBeenCalledOnce();
    const arg = createMock.mock.calls[0][0] as { data: Record<string, unknown> };
    expect(arg.data.userSub).toBe("u1");
    expect(arg.data.workflowKey).toBe("generic-alert");
    expect(arg.data.title).toBe("T");
    expect(sendEmailMock).not.toHaveBeenCalled();
  });

  it("sends email when enabled with email channel and an address", async () => {
    findUniqueMock.mockResolvedValue({ key: "welcome", enabled: true, channels: ["email"] });
    await notify("welcome", { userSub: "u1", email: "a@b.com", data: { body: "hi" } });
  ...
  it("never throws when the DB call fails", async () => {
    ...
    await expect(notify("generic-alert", { userSub: "u1", data: {} })).resolves.toBeUndefined();
  });
